...
import {
  processImageWithFawkes,
  processImageWithAdvCloak,
  cleanup,
} from "../utils/cloakingEngine";
...
export function useImageCloaking() {
    ...
    const initWorker = () => {
      try {
        workerRef.current = new Worker(
          new URL("../utils/cloakingWorker.js", import.meta.url),
        );
        ...
        logger.warn(
          "Worker initialization failed, using main thread only:",
          error,
        );
        workerRef.current = null;
        workerAvailable.current = false;
  ...
  const processImage = useCallback(async (imageFile, settings) => {
    ...
    try {
      ...
      try {
        const estimate = await processingEstimator.estimateProcessingTime(
          imageFile,
          settings,
        );
      ...
      if (
        ...
        try {
          ...
          if (workerError.message === "FALLBACK_TO_MAIN_THREAD") {
            logger.info(
              "Falling back to main thread processing due to worker backend issues",
            );
        ...
        const messageHandler = (e) => {
          ...
          if (e.data.type === "result" && !completed) {
            ...
            if (
              ...
              logger.info(
                "Backend error detected, falling back to main thread",
              );
              workerAvailable.current = false;
  ...
  const processOnMainThread = useCallback(async (imageDataURL, settings) => {
    ...
    try {
      if (settings.method === "fawkes") {
        ...
        result = await processImageWithFawkes(
          imageDataURL,
  ...
  const handleProcessingComplete = useCallback(async (result) => {
    ...
    if (
      ...
      try {
        ...
        logger.debug(
          `Processing completed: estimated ${estimatedProcessingTime.current}s, actual ${actualTime}s`,
        );
      } catch (error) {
    ...
    try {
      ...
      if (originalImageDataURL.current && result.imageDataURL) {
        ...
        logger.warn(
          "Cannot calculate metrics: missing original or processed image",
          {
