    ...
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-xs font-mono">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={getPerformanceColor(throttleStats.performanceGrade)}>
            {getPerformanceIcon(throttleStats.performanceGrade)}{" "}
            {throttleStats.performanceGrade}
          </span>
      ...
      {isExpanded && (
        <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1">
          <div>
            FPS:{" "}
            <span
              className={getPerformanceColor(throttleStats.performanceGrade)}
            >
          ...
          <div>
            Memory Pressure: {(throttleStats.memoryPressure * 100).toFixed(1)}%
          </div>

          <div>Frames: {throttleStats.frameCount}</div>
          <div>
            Yields:{" "}
            <span className="text-blue-600">{throttleStats.yieldCount}</span>
          </div>
        </div>
      )}
      ...
      <div className="mt-2 flex gap-1">
        ...
        <div className="flex-1 h-1 bg-gray-200 rounded">
          <div
            className={`h-full rounded transition-all duration-300 ${
              throttleStats.realFPS > 50
                ? "bg-green-500"
                : throttleStats.realFPS > 40
                  ? "bg-blue-500"
                  : throttleStats.realFPS > 25
                    ? "bg-yellow-500"
                    : "bg-red-500"
            }`}
            style={{
              width: `${Math.min(100, (throttleStats.realFPS / 60) * 100)}%`,
            }}
          />
        </div>
        ...
        <div className="flex-1 h-1 bg-gray-200 rounded">
          <div
            className={`h-full rounded transition-all duration-300 ${
              throttleStats.memoryPressure < 0.3
                ? "bg-green-500"
                : throttleStats.memoryPressure < 0.5
                  ? "bg-blue-500"
                  : throttleStats.memoryPressure < 0.8
                    ? "bg-yellow-500"
                    : "bg-red-500"
            }`}
            style={{
              width: `${Math.min(100, throttleStats.memoryPressure * 100)}%`,
            }}
          />
        </div>
        ...
        <div className="flex-1 h-1 bg-gray-200 rounded">
          <div
            className={`h-full rounded transition-all duration-300 ${
              throttleStats.processingTime < throttleStats.frameBudget * 0.5
                ? "bg-green-500"
                : throttleStats.processingTime < throttleStats.frameBudget * 0.8
                  ? "bg-blue-500"
                  : throttleStats.processingTime < throttleStats.frameBudget
                    ? "bg-yellow-500"
                    : "bg-red-500"
            }`}
            style={{
