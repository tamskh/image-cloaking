...
export function downloadImage(dataURL, filename) {
  try {
    ...
    logger.debug(
      `Attempting to download image, data URL length: ${dataURL.length}`,
    );
...
function calculateSSIM(original, processed) {
  ...
  for (let i = 0; i < origData.length; i += 4) {
    ...
    const x =
      0.299 * origData[i] + 0.587 * origData[i + 1] + 0.114 * origData[i + 2];
    const y =
      0.299 * procData[i] + 0.587 * procData[i + 1] + 0.114 * procData[i + 2];
...
export async function calculateImageMetrics(originalDataURL, processedDataURL) {
  try {
    ...
    if (
      originalImageData.width !== processedImageData.width ||
      ...
      finalOriginalImageData = ctx.getImageData(
        0,
        0,
        processedImageData.width,
        processedImageData.height,
    ...
    const metrics = {
      ...
      perceptual_distance: calculatePerceptualDistance(
        finalOriginalImageData,
        finalProcessedImageData,
      ),
    };
