...
export class ProcessingTimeEstimator {
  ...
  async estimateProcessingTime(image, settings) {
    try {
      ...
      switch (settings.method) {
        case 'fawkes':
          totalTime += this.estimateFawkesTime(pixelCount, settings.fawkesLevel, complexityFactor, rateMultiplier)
          break
          
        case 'advcloak':
          totalTime += this.estimateAdvCloakTime(pixelCount, settings, complexityFactor, rateMultiplier)
          break
      ...
      const calibratedTime = this.applyCalibration(totalTime, settings.method, pixelCount)
  ...
  estimateFawkesTime(pixelCount, level, complexityFactor, rateMultiplier) {
    // Face detection time
    const faceDetectionTime = pixelCount / (PROCESSING_RATES.faceDetection[this.device.tier] * rateMultiplier)
    
    // Face processing time (depends on protection level)
    const faceProcessingRate = PROCESSING_RATES.fawkesBase[level] || PROCESSING_RATES.fawkesBase.mid
    const faceProcessingTime = pixelCount / (faceProcessingRate * rateMultiplier * complexityFactor)
    
    return faceDetectionTime + faceProcessingTime + PROCESSING_RATES.overhead.faceProcessing
  }
  ...
  estimateAdvCloakTime(pixelCount, settings, complexityFactor, rateMultiplier) {
    ...
    const iterationRate = PROCESSING_RATES.advCloakIteration[this.device.tier] * rateMultiplier
    ...
    const totalIterationTime = timePerIteration * iterations * Math.max(iterationEfficiency, 0.7)
    
    // Epsilon affects complexity (lower epsilon = more iterations needed effectively)
    const epsilonFactor = Math.max(0.8, Math.min(1.3, 0.05 / (settings.advCloakEpsilon || 0.05)))
  ...
  recordActualTime(method, estimatedTime, actualTime, pixelCount) {
    ...
    try {
      ...
      if (!this.calibrationData[key]) {
        ...
        cal.estimatedAvg = (cal.estimatedAvg * (newSamples - 1) + estimatedTime) / newSamples
        cal.actualAvg = (cal.actualAvg * (newSamples - 1) + actualTime) / newSamples
        cal.samples = newSamples
      }
      
      localStorage.setItem('cloaking_calibration', JSON.stringify(this.calibrationData))
    } catch (error) {
...
export async function getProcessingTimeEstimate(image, settings) {
  const estimatedSeconds = await processingEstimator.estimateProcessingTime(image, settings)
  return {
