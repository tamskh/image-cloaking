...
export class PerformanceOptimizer {
  ...
  getRecommendedMaxImageSize() {
    switch (this.deviceCapabilities.tier) {
      case 'high': return { width: 4096, height: 4096, fileSize: 25 * 1024 * 1024 }
      case 'medium': return { width: 2048, height: 2048, fileSize: 10 * 1024 * 1024 }
      case 'low': return { width: 1024, height: 1024, fileSize: 5 * 1024 * 1024 }
      default: return { width: 1024, height: 1024, fileSize: 5 * 1024 * 1024 }
    }
  }
  ...
  getMemoryManagementTips() {
    return {
      ...
      recommendedCleanupInterval: this.deviceCapabilities.memory < 4 ? 30000 : 60000
    }
  }
  ...
  calculateTrend(values) {
    ...
    const earlierAvg = values.slice(0, -3).reduce((a, b) => a + b, 0) / (values.length - 3)
  ...
  generatePerformanceReport() {
    ...
    } else if (trends.filter(t => t === 'improving').length > trends.length * 0.3) {
  ...
  static formatDuration(ms) {
    ...
    else return `${Math.floor(ms / 60000)}m ${Math.round((ms % 60000) / 1000)}s`
  }
