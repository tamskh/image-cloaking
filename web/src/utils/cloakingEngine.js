    ...
    try {
      ...
      if (originalBackend !== "cpu") {
        try {
          ...
          logger.warn(
            `Failed to restore ${originalBackend} backend:`,
            backendError,
          );
        }
      }
  ...
  createSurrogateModel(inputShape) {
    try {
      ...
      if (imageSize > 2048 * 2048) {
        ...
        kernelSize2 = 3;
        logger.info(
          `Using minimal surrogate model for large image: ${width}×${height}`,
        );
      ...
      logger.debug(
        `Surrogate model created: ${filters1}-${filters2}-${filters3} filters, ${kernelSize1}×${kernelSize2} kernels`,
      );
      return model;
    } catch (error) {
      ...
      try {
        ...
        if (i % 5 === 0) {
          ...
          if (memInfo.numBytes > 800 * 1024 * 1024) {
            // > 800MB - same as worker
            logger.warn(
              `High memory usage: ${Math.round(memInfo.numBytes / 1024 / 1024)}MB, forcing cleanup`,
            );
...
export async function processImageWithFawkes(
  imageDataURL,
  protectionLevel = "mid",
  progressCallback,
  abortSignal,
  ...
  try {
    ...
    if (faces.length === 0) {
      throw new Error(
        "No faces detected in the image. Fawkes protection requires faces to be present.",
      );
    }
    ...
    try {
      ...
      if (
        ...
        try {
          const cpuAttack = new IterativeFGSMAttack(
            epsilon,
          ...
          perturbation =
            await simpleCpuAttack.generatePerturbationCPU(imageTensor);
          progressCallback?.(90);
        }
      } else {
        throw error;
    ...
    logger.debug(
      `Tensor range before conversion: [${minVal[0].toFixed(3)}, ${maxVal[0].toFixed(3)}]`,
    );
...
export async function processImageWithAdvCloak(
  imageDataURL,
  epsilon = 0.04,
  iterations = 12,
  progressCallback,
  ...
  try {
    ...
    const maxVal = await clippedTensor.max().data();
    logger.debug(
      `AdvCloak tensor range before conversion: [${minVal[0].toFixed(3)}, ${maxVal[0].toFixed(3)}]`,
    );
    ...
    img.onload = () => {
      try {
        ...
        if (
          ...
          logger.info(
            `Resizing image from ${originalWidth}×${originalHeight} to ${finalWidth}×${finalHeight} to prevent WebGL texture overflow`,
          );
        }
        ...
        logger.debug(
          `Input tensor range: [${minVal.toFixed(3)}, ${maxVal.toFixed(3)}], shape: ${tensor.shape}`,
        );

        if (minVal < -1.1 || maxVal > 1.1) {
          logger.warn(
            `Tensor values outside expected [-1, 1] range: [${minVal}, ${maxVal}]`,
          );
        ...
        reject(
          new Error("Image to tensor conversion failed: " + error.message),
        );
      }
    };
    img.onerror = (error) => {
...
async function tensorToImageDataURL(tensor) {
  try {
    ...
    logger.debug(
      `Image converted: ${width}x${height}, estimated size: ${sizeEstimate.toFixed(2)}MB, quality: ${Math.round(quality * 100)}%`,
    );
