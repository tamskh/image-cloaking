...
const logger = {
  debug: (message, ...args) =>
    console.log(`[CloakingWorker] DEBUG: ${message}`, ...args),
  info: (message, ...args) =>
    console.info(`[CloakingWorker] ${message}`, ...args),
  warn: (message, ...args) =>
    console.warn(`[CloakingWorker] ${message}`, ...args),
  error: (message, ...args) =>
    console.error(`[CloakingWorker] ${message}`, ...args),
};
...
function logMemoryUsage(context = "") {
  ...
  if (memoryMB > 200) {
    // Log if > 200MB (lower threshold)
    logger.warn(
      `${context} Memory usage: ${memoryMB.toFixed(2)} MB (${memInfo.numTensors} tensors)`,
    );
  }
}
...
async function initializeWorker() {
  ...
  try {
    ...
    isInitialized = true;
    logger.info(
      `Worker initialized successfully with ${tf.getBackend()} backend`,
    );
  } catch (error) {
...
class WorkerFGSMAttack {
  ...
  createSurrogateModel(inputShape) {
    try {
      ...
      if (imageSize > 2048 * 2048) {
        ...
        kernelSize2 = 3;
        logger.info(
          `Worker: Using minimal surrogate model for large image: ${width}×${height}`,
        );
      ...
      logger.debug(
        `Worker surrogate model: ${filters1}-${filters2}-${filters3} filters, ${kernelSize1}×${kernelSize2} kernels`,
      );
      return model;
    } catch (error) {
  ...
  async generatePerturbation(imageTensor, targetClass = null) {
    ...
    try {
      ...
      logger.error(
        `Worker FGSM perturbation generation failed: ${error.message}`,
      );
    ...
    for (let i = 0; i < this.iterations; i++) {
      ...
      try {
        ...
        const newAdversarial = tf.tidy(() => {
          ...
          const clippedPerturbation = tf.clipByValue(
            perturbation,
            -this.epsilon,
            this.epsilon,
          );
        ...
        if (i % 5 === 0) {
          // Every 5 iterations instead of 3
          // Check memory usage
          const memInfo = tf.memory();
          if (memInfo.numBytes > 800 * 1024 * 1024) {
            // > 800MB instead of 500MB
            logger.warn(
              `Worker high memory usage: ${Math.round(memInfo.numBytes / 1024 / 1024)}MB, forcing cleanup`,
            );
        ...
        if (i % 3 === 0 || processingTime > 16) {
          // Every 3 iterations or if > 16ms (one frame)
...
async function imageDataURLToTensor(dataURL) {
  try {
    ...
    if (
      ...
      logger.info(
        `Worker: Resizing image from ${originalWidth}×${originalHeight} to ${finalWidth}×${finalHeight} to prevent WebGL texture overflow`,
      );
    }
...
async function tensorToImageDataURLFallback(tensor) {
  ...
  try {
    ...
    while (blob.size > targetMaxSize && quality > 0.7) {
      quality -= 0.05;
...
async function tensorToImageDataURL(tensor) {
  ...
  try {
    ...
    if (squeezed.rank !== 3) {
      throw new Error(
        `Expected 3D tensor, got ${squeezed.rank}D. Shape: ${squeezed.shape}`,
      );
    }
    ...
    logger.debug(
      `Normalized value range: [${minVal[0].toFixed(3)}, ${maxVal[0].toFixed(3)}]`,
    );
    ...
    if (width > maxDimension || height > maxDimension) {
      ...
      finalHeight = Math.round(height * scale);
      logger.debug(
        `Resizing from ${width}x${height} to ${finalWidth}x${finalHeight}`,
      );
    }
    ...
    if (finalWidth !== width || finalHeight !== height) {
      ...
      const resized = tf.image.resizeBilinear(normalized, [
        finalHeight,
        finalWidth,
      ]);
    ...
    logger.debug(
      `Initial blob size: ${(blob.size / 1024 / 1024).toFixed(2)}MB at ${Math.round(quality * 100)}% quality`,
    );
    ...
    while (blob.size > targetMaxSize && quality > 0.7) {
      ...
      logger.debug(
        `Adjusted to ${(blob.size / 1024 / 1024).toFixed(2)}MB at ${Math.round(quality * 100)}% quality`,
      );
    }
    ...
    if (quality < 0.7) {
      logger.warn(
        "Quality dropped below 70%, using minimum acceptable quality",
      );
      blob = await canvas.convertToBlob({
        type: "image/jpeg",
    ...
    logger.debug(
      `Final optimized size: ${(blob.size / 1024 / 1024).toFixed(2)}MB at ${Math.round(quality * 100)}% quality`,
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        if (result.length > maxDataURLSize) {
          logger.warn(
            `Data URL size: ${(result.length / 1024 / 1024).toFixed(2)}MB exceeds target but proceeding`,
          );
    ...
    logger.warn(
      "Primary tensor conversion failed, trying fallback:",
      error.message,
    );
    ...
    try {
      ...
      logger.error(
        "Both primary and fallback conversion failed:",
        fallbackError,
      );
      throw new Error(
        `Failed to convert tensor to image: ${error.message}. Fallback also failed: ${fallbackError.message}`,
      );
    }
  } finally {
...
async function processWithFawkes(data, taskId) {
  ...
  try {
    ...
    logger.debug(
      `Final tensor value range: [${finalMin[0].toFixed(3)}, ${finalMax[0].toFixed(3)}]`,
    );
