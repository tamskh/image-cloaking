...
export class ProcessingThrottle {
  ...
  updatePerformanceGrade() {
    ...
    this.memoryPressure = Math.max(
      0,
      (memoryUsage - this.memoryBaseline) / this.memoryBaseline,
    );
  ...
  getBaseYieldTime() {
    switch (this.performanceGrade) {
      case "excellent":
        return 0; // Immediate yield
      case "good":
        return 1;
      case "fair":
        return 5;
      case "poor":
        return 16; // Full frame yield
      default:
        return 5;
    }
  }
...
export async function processInChunks(array, chunkProcessor, options = {}) {
  const {
    initialChunkSize = 10,
    maxChunkSize = 100,
    minChunkSize = 1,
  ...
  for (let i = 0; i < array.length; i += adaptiveChunkSize) {
    ...
    if (processingTime < throttle.frameBudget * 0.5) {
      ...
      adaptiveChunkSize = Math.min(
        maxChunkSize,
        Math.ceil(adaptiveChunkSize * 1.2),
      );
    } else if (processingTime > throttle.frameBudget * 0.8) {
      // Processing was slow, decrease chunk size
      adaptiveChunkSize = Math.max(
        minChunkSize,
        Math.ceil(adaptiveChunkSize * 0.8),
      );
    }
...
export async function processTensorInBatches(
  ...
  for (let i = 0; i < totalBatches; i++) {
    ...
    if (
      throttle.shouldYield(processingStart) ||
      throttle.memoryPressure > 0.5
    ) {
